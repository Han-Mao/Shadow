...
def _point_to_pixel(adb: AdbController, x: float, y: float) -> tuple[int, int]:
    """像素坐标直接取整，不进 ADB（省一次 dumpsys）；只有归一化坐标才查屏幕尺寸。"""
...
def _bbox_center(adb: AdbController, box: list[float]) -> tuple[int, int]:
    x1, y1, x2, y2 = box
...
def resolve_target(
    ...
    if isinstance(target, Point):
        return _point_to_pixel(adb, target.x, target.y)
    ...
    if isinstance(target, str):
        ...
        if numbers:
            if len(numbers) == 4:
                return _bbox_center(adb, numbers)
            return _point_to_pixel(adb, numbers[0], numbers[1])
