...
class PrintedDevice:
    ...
    def current_focus(self) -> tuple[str, str]:
        return ("com.taobao.taobao", ".Main")

    def deadline_budget(self, seconds: float):
        """给 `observer` 的采集预算用的上下文管理器（假设备就什么都不做）。"""
        import contextlib
...
def install_stubs(tmp: Path) -> None:
    ...
    def fake_plan_next(
        instruction, screenshot_path, ui_tree=None, history=None, plan=None, current_step=None,
        plan_context="",
    ):
...
def main() -> None:
    # 本脚本演示的是「抢占与恢复」，不是「完成验证」——后者有专门的测试覆盖
    # （tests/test_goal_policy.py / test_goal_verifier.py）。设 advisory 让被打断后
    # 恢复的任务能干净走完，而不是因为「计划未完成 + 无证据」被 strict 卡进人工。
    import os

    os.environ["GOAL_VERIFY_MODE"] = "advisory"
